...
Six games approximately represent one week of MLB play and provide a practical balance between short-window variability and roster changes accumulated over longer windows. Six games give players several batting opportunities and also limit the number of roster changes. A longer window would include more roster changes and more injuries, while a shorter one could introduce more variance.
...
- Stolen-base runs were calculated using wSB. This was used instead of a different statistic like Ultimate Base Running (UBR) [@fangraphs_ubr] due to differences in how baserunning value is assigned. Additionally, the sprinter will only be assigned stolen-base runs, so it evens out between both options.
...
We model the race margin as a logistic distribution centred on the 0.295 second margin above, and we assign it a scale parameter of 0.113 seconds. This scale was found from observing tracked throws and stolen base attempts to find an estimate for variance in stolen base attempts. Under these timing and variance assumptions, the model implies a 93.1% safe rate visible in @fig-race.
...
read_csv(file.path("output", "reproducibility", "game_state_decisions.csv"),
  ...
  kable(format = "latex", booktabs = TRUE, linesep = "", align = c("r", "r", "l"))
```
